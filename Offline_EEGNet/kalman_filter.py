...
def weighted_inner_product(x, y, sample_weight=None):
    '''returns (x.T*sample_weight)@y'''
...
class KalmanFilter():
    def __init__(self, state_dim=7, obs_dim=2, dt=0.020, lam=0.999, gain=1.0):
        ...
        self.state[-1] = 1.0
        return
    def fit(self, states, obss, prev_states=None, sample_weight=None, max_iter=100, EBS=None):
    ...
    def rescale_EBS(self, EBS):
        if EBS is None:
            return
        # self.R == self.EBS * weighted_expectation(x@x.T)
    ...
    def process_state_obs(self, state, obs, iterate_inv=True, kf_iter=1):
        '''
