...
def shm_setup(signals, module):
    '''
    Setup shared memory using existing SharedMemory files.
...
def verify_trigger(modules):
    '''
    Checks if each module path has a trigger.
    '''
...
def verify_trigger_module(modules, name, origin=None):
    '''
    Checks if an module has a trigger in its upstream connected graph
    '''
