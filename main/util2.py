...
class RaspySharedMemoryManager():
    ...
    '''
    def __init__(self, signals, pid=None):
        ...
        self.shm_dict = self.init_shm(signals, pid)
        self.pid = pid
        return
    def __del__(self):
        for key, value in self.shm_dict.items():
            self.shm_dict[key].close()
            self.shm_dict[key].unlink()
        return
    @staticmethod
    def init_shm(signals, pid=None):
        '''
        This is here if you really need it but it should probably only be run from each instance's init.
        '''
        ...
        for name, signal in signals.items():
            ...
            shm_name = name + pid_str
            try:
                shm_dict[name] = mp.shared_memory.SharedMemory(name=shm_name, create=True, size=nbytes)
            except:
                # removes and recreates shared memory objects
                # THIS WILL CAUSE PROBLEMS IF YOU TRY TO RUN TWO MODULES WHICH SHARE A VARIABLE
                # UNLESS YOU SPECIFY PID.
                # IF YOU SPECIFY PID AND THIS DOES NOT EXIT CLEANLY THEN YOU WILL HAVE A STRAY MEMORY OBJECT
                # You should probably only use this within a SharedMemoryManager object.
                tmp = mp.shared_memory.SharedMemory(name=shm_name, create=False)
                tmp.unlink()
                shm_dict[name] = mp.shared_memory.SharedMemory(name=shm_name, create=True, size=nbytes)
            ## Initialize to zeros. On Linux, Windows, Mac this is not necessary, so it's commented out.
            #shm_dict[name].buf[:] = bytes(len(shm_dict[name].buf))
        return shm_dict

def shm_setup(signals, module, pid=None):
